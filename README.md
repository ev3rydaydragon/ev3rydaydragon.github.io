...
- **Preact 10 + `preact/compat`** — self-hosted under `assets/vendor/preact-react.min.js` and aliased to `window.React`/`window.ReactDOM`, so the Design Composer runtime renders through it unchanged while shipping a fraction of React's bytes. Self-hosted rather than loaded from a CDN at runtime, so the homepage doesn't depend on a third party being reachable to boot (see Performance notes below).
...
- **Preact instead of React (self-hosted)**: The runtime originally pulled React + ReactDOM from `unpkg.com`, then those were vendored locally (142 KB raw / ~47 KB gzip across two files). The Design Composer runtime only touches a small, stable slice of the API (`createElement`, `Component`, `Fragment`, `isValidElement`, `useState`/`useEffect`/`useMemo`, `createContext`, `memo`, and `ReactDOM.render` — no React-18 concurrent features), so it now renders on **Preact 10 + `preact/compat`** aliased to `window.React`/`window.ReactDOM`. That's a single self-hosted file, `assets/vendor/preact-react.min.js` (**27 KB raw / ~10 KB gzip — about 37 KB less over the wire, one request instead of two**). The swap was validated in a headless browser: the homepage renders behaviorally identical to the React build across the typing intro, every scroll scene, the doors/CTA/footer, and the mobile + `prefers-reduced-motion` paths, with zero console errors. `support.js`'s built-in "load React from a CDN if `window.React` is missing" fallback short-circuits because the alias is set first, so nothing hits the network for React.
- **Render-blocking Resources**: `support.js` and the vendored Preact script use the `defer` attribute. This allows the browser to parse the DOM without waiting for JavaScript execution, improving Time to First Byte (TTFB) and First Contentful Paint (FCP).
